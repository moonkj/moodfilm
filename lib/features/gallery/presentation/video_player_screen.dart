...
class _VideoPlayerScreenState extends ConsumerState<VideoPlayerScreen> {
  ...
  bool _showSplit = false; // 비교 모드 (필터 이름 오버레이)
    ...
    try {
      final result = await FilterEngine.processVideo(
        ...
        effects: {
          ...
          'saturation': _saturation, 'beauty': _beauty,
          'fade': _fade, 'dreamyGlow': _dreamyGlow,
        },
        saveToGallery: false,
      );
      if (!mounted) return;
                  ...
                  child: Container(
                    ...
                    child: Stack(
                      ...
                      children: [
                        ...
                        // 비교 모드: 필터 이름 오버레이
                        if (_showSplit && !_noFilter && camera.activeFilter != null)
                          Positioned(
                            top: 12, left: 12,
                            child: Container(
  ...
  Widget _buildTopBar(dynamic camera) {
    return Padding(
      ...
      child: Row(
        children: [
          IconButton(
            icon: const Icon(Icons.arrow_back_ios_rounded, color: Color(0xFF3D3531), size: 20),
  ...
  Widget _topIconBtn(IconData icon, {required VoidCallback onTap, bool active = false}) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Icon(icon, color: active ? Colors.white : const Color(0xFF3D3531), size: 18),
      ),
    );
  }
          ...
          return GestureDetector(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(param.label,
                    style: TextStyle(
                      color: isActive ? const Color(0xFFB06878) : const Color(0xFF8A8480),
                      fontSize: 11,
  ...
  Widget _buildBottomTabBar(dynamic camera) {
    final hasFilterChange = !_noFilter;
    return Container(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
        children: [
          _buildTab('필터', Icons.auto_awesome_rounded, 'filter', hasDot: hasFilterChange),
  ...
  Widget _buildTab(String label, IconData icon, String tabId, {bool hasDot = false}) {
